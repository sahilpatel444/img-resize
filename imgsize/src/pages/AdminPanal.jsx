...
// delete message 
  ...
  useEffect(() => {
    ...
    const fetchMessages = async () => {
      try {
        const res = await axios.get(`${import.meta.env.VITE_BLACKEND_URL}/api/auth/messages`);
  ...
  const handleAddNavbarItem = async () => {
    ...
    try {
      ...
      console.error("Error adding navbar item:", error.response?.data || error.message);
      if (error.response?.data?.logout) {
        ClearLocalStorage(); // Call the logout function
      }
  ...
  const handleUpdateNavbarItem = async () => {
    ...
    try {
      await axios.put(
        `${import.meta.env.VITE_BLACKEND_URL}/api/admin/navbar/${editingItem._id}`,
        editingItem,
        { headers: { Authorization: `Bearer ${token}` } },
      );
  ...
  <div className="w-full lg:w-1/3 bg-white shadow-md p-5 rounded-lg">
    ...
    <div className="max-h-48 overflow-y-scroll pr-3 scrollbar-hide mt-4">
      ...
      {messages.length === 0 ? (
        ...
        messages.map((msg) => (
          <div key={msg._id} className="p-3 border rounded-lg mb-3 flex justify-between items-center bg-gray-50">
            <div>
      ...
      <div className="overflow-x-auto">
        ...
        <div
          ref={tableRef}
          className="overflow-y-auto max-h-96 border border-gray-300 rounded-lg shadow-md"
